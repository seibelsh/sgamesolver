...
For this quick example, we are using the method :py:meth:`random_game` to
randomize a game with 64 states, 2 players, and 4 actions per player and state.
...
Section :doc:`defining_games` contains instructions and examples
on how to create an :py:class:`SGame`
...
Some details can be found in the section :doc:`quantal_response_equilibrium`.
...
also see the section :doc:`logarithmic_tracing_procedure`.

Which homotopy to pick?
In our experience, QRE is more robust
while LogTracing has the advantage that it
